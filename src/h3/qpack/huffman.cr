    ...
    module Huffman
      ...
      def self.decode(bytes : Bytes) : String
        ...
        bytes.each do |b|
          ...
          raise "Huffman Decode Error at byte #{b.to_s(16)} (high nibble), state #{state}" if (node[1] & 4) != 0
          io.write_byte(node[2].to_u8) if (node[1] & 2) != 0
          state = node[0].to_u16

          nibble2 = b & 0x0F
          node = TABLE[(state << 4) + nibble2]
          raise "Huffman Decode Error at byte #{b.to_s(16)} (low nibble), state #{state}" if (node[1] & 4) != 0
          io.write_byte(node[2].to_u8) if (node[1] & 2) != 0
          state = node[0].to_u16
        end
